...
async function getAllOwnedRepos() {
  const repos = [];
  for (let page = 1; ; page++) {
    const batch = await request(`/user/repos?per_page=100&page=${page}&type=owner&sort=updated`);
...
const contributedTo = contributions.totalRepositoriesWithContributedCommits;

// Transparent profile score based only on live GitHub activity.
...
<style>
.bg{fill:#0d1117}.title{fill:#ff69b4;font-size:24px;font-weight:700;font-family:'Segoe UI',sans-serif}.label{fill:#ffc0cb;font-size:17px;font-family:'Segoe UI',sans-serif}.value{fill:#fff;font-weight:700}.muted{fill:#8b949e;font-size:11px;font-family:'Segoe UI',sans-serif}.ring-bg{stroke:#2a2a2a;stroke-width:11;fill:none}.ring{stroke:#ff69b4;stroke-width:11;fill:none;stroke-linecap:round}
</style>
...
<text x="45" y="112" class="label">⭐ Stars: <tspan class="value">${stars}</tspan></text>
<text x="45" y="150" class="label">📦 Commits (${YEAR}): <tspan class="value">${commits}</tspan></text>
<text x="45" y="188" class="label">🔀 PRs (${YEAR}): <tspan class="value">${prs}</tspan></text>
<text x="45" y="226" class="label">🐞 Issues (${YEAR}): <tspan class="value">${issues}</tspan></text>
<text x="45" y="264" class="label">🤝 Contributed to: <tspan class="value">${contributedTo}</tspan></text>
...
<text x="380" y="307" text-anchor="middle" class="muted">Last API update: ${esc(updated)}</text>
</svg>`;

fs.mkdirSync("assets", { recursive: true });
fs.writeFileSync("assets/github-stats.svg", svg);
console.log({ YEAR, stars, commits, prs, issues, contributedTo, score, grade, updated });
